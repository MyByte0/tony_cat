...
class NetPbModule : public ModuleBase {
 ...
 public:
    ...
    bool WriteData(Session::session_id_t sessionId, uint32_t msgType,
    ...
    struct GetFunctionMessage<_TypeRet(Session::session_id_t, _TypeHead&,
                                       _TypeMessage&)> {
    ...
    struct GetFunctionMessage<_TypeRet (_Fn::*)(Session::session_id_t,
                                                _TypeHead&, _TypeMessage&)> {
        using TypeHead = _TypeHead;
        using TypeBody = _TypeMessage;
        using TypeRet = _TypeRet;
    };
 ...
 private:
    ...
    FuncPacketHandleType GeneratePaserPacketFunc(
        const std::function<_TypeRet(
            Session::session_id_t sessionId, _TypeMsgPacketHead& packetHead,
            _TypeMsgPacketBody& packetBody)>& funReadPacket) {
    ...
    void RegisterHandleHelper(
        uint32_t msgType,
        const std::function<_TypeRet(Session::session_id_t sessionId,
                                     _TypeMsgPacketHead& packetHead,
                                     _TypeMsgPacketBody& packetBody)>& func) {
 ...
 public:
    ...
    bool SendPacket(Session::session_id_t sessionId,
        ...
        char buffHead[sizeof(uint32_t) + kPacketHeadMaxLen +
                      kPacketBodyMaxCache];
        char* pData = buffHead;
        ...
        SetPacketHeadLength(reinterpret_cast<uint32_t*>(pData),
                            pbPacketHeadLen);
        ...
        if (pbPacketBodyLen + pbPacketHeadLen + sizeof(uint32_t) <
            sizeof buffHead) {
            ...
            return WriteData(sessionId, msgType, buffHead,
                             pbPacketHeadLen + sizeof(uint32_t),
                             strPacketBody.data(), strPacketBody.size());
        }
    }

    template <class _TypeMsgPacketHead>
    bool SendPacket(Session::session_id_t sessionId, uint32_t msgType,
                    _TypeMsgPacketHead& packetHead, const char* data,
                    std::size_t length) {
        FillHeadCommon(packetHead);
        char buffHead[sizeof(uint32_t) + kPacketHeadMaxLen +
                      kPacketBodyMaxCache];
        ...
        pData += pbPacketHeadLen;
        return WriteData(sessionId, msgType, buffHead,
                         pbPacketHeadLen + sizeof(uint32_t), data, length);
    }
    ...
    void RegisterHandle(_TypeClass* pClass,
                        _TypeRet (_TypeClass::*func)(Session::session_id_t,
                                                     _TypeMsgPacketHead&,
                                                     _TypeMsgPacketBody&)) {
        std::function<_TypeRet(Session::session_id_t, _TypeMsgPacketHead&,
                               _TypeMsgPacketBody&)>
